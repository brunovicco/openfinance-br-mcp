"""Treasure Titles API - transactions_current operation module: Obtém as movimentações recentes da operação de Títulos do Tesouro Direto identificada por"""
...
from clients.treasure_titles_v1_1_0.client import AuthenticatedClient, Client
...
from clients.treasure_titles_v1_1_0.types import UNSET, Response, Unset
...
) -> ResponseError | ResponseTreasureTitlesProductTransactions:
    if response.status_code == 200:
        response_200 = ResponseTreasureTitlesProductTransactions.from_dict(
            response.json()
        )
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseError | ResponseTreasureTitlesProductTransactions]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém as movimentações recentes da operação de Títulos do Tesouro Direto identificada por
    investmentId. O período a ser considerado para apresentação de movimentações será de até 7 dias - 7
    dias anteriores da consulta, incluindo o dia da consulta (D-6).
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: str,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseError | ResponseTreasureTitlesProductTransactions | None:
    """Obtém as movimentações recentes da operação de Títulos do Tesouro Direto identificada por
    investmentId. O período a ser considerado para apresentação de movimentações será de até 7 dias - 7
    dias anteriores da consulta, incluindo o dia da consulta (D-6).
