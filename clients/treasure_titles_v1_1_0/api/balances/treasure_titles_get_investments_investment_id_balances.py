"""Treasure Titles API - balances operation module: Obtém a posição da operação de Títulos do Tesouro Direto identificada por investmentId."""
...
) -> ResponseError | ResponseTreasureTitlesBalances:
    if response.status_code == 200:
        response_200 = ResponseTreasureTitlesBalances.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseError | ResponseTreasureTitlesBalances]:
    return Response(
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: str,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseError | ResponseTreasureTitlesBalances | None:
    """Obtém a posição da operação de Títulos do Tesouro Direto identificada por investmentId.
