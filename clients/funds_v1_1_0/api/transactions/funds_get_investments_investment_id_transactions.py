"""Funds API - transactions operation module: Obtém as movimentações históricas (últimos 12 meses) da operação de Fundos de Investimento"""
...
) -> ResponseErrorMetaSingle | ResponseFundsTransactions:
    if response.status_code == 200:
        response_200 = ResponseFundsTransactions.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseErrorMetaSingle | ResponseFundsTransactions]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém as movimentações históricas (últimos 12 meses) da operação de Fundos de Investimento
    identificada por investmentId.
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: str,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseErrorMetaSingle | ResponseFundsTransactions | None:
    """Obtém as movimentações históricas (últimos 12 meses) da operação de Fundos de Investimento
    identificada por investmentId.
