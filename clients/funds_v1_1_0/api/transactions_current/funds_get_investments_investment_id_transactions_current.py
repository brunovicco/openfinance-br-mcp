"""Funds API - transactions_current operation module: Obtém as movimentações recentes da operação de Fundos de Investimento identificada por investmentId."""
...
from clients.funds_v1_1_0.models.response_funds_transactions_current import (
...
def _get_kwargs(
    ...
    *,
    ...
    from_transaction_conversion_date: datetime.date | Unset = UNSET,
    to_transaction_conversion_date: datetime.date | Unset = UNSET,
    authorization: str,
    ...
    json_from_transaction_conversion_date: str | Unset = UNSET
    if not isinstance(from_transaction_conversion_date, Unset):
        json_from_transaction_conversion_date = (
            from_transaction_conversion_date.isoformat()
        )
    params["fromTransactionConversionDate"] = json_from_transaction_conversion_date

    json_to_transaction_conversion_date: str | Unset = UNSET
    if not isinstance(to_transaction_conversion_date, Unset):
        json_to_transaction_conversion_date = to_transaction_conversion_date.isoformat()
    params["toTransactionConversionDate"] = json_to_transaction_conversion_date
...
) -> ResponseErrorMetaSingle | ResponseFundsTransactionsCurrent:
    if response.status_code == 200:
        response_200 = ResponseFundsTransactionsCurrent.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseErrorMetaSingle | ResponseFundsTransactionsCurrent]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém as movimentações recentes da operação de Fundos de Investimento identificada por investmentId.
    O período a ser considerado para apresentação de movimentações será de até 7 dias - 7 dias
    anteriores da consulta, incluindo o dia da consulta (D-6).
    ...
    Args:
        ...
        from_transaction_conversion_date (datetime.date | Unset):  Example: 2021-05-21.
        to_transaction_conversion_date (datetime.date | Unset):  Example: 2021-05-21.
        authorization (str):
    ...
    kwargs = _get_kwargs(
        ...
        from_transaction_conversion_date=from_transaction_conversion_date,
        to_transaction_conversion_date=to_transaction_conversion_date,
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: str,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseErrorMetaSingle | ResponseFundsTransactionsCurrent | None:
    """Obtém as movimentações recentes da operação de Fundos de Investimento identificada por investmentId.
    O período a ser considerado para apresentação de movimentações será de até 7 dias - 7 dias
    anteriores da consulta, incluindo o dia da consulta (D-6).
