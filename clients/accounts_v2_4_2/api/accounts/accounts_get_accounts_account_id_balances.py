"""Accounts API - accounts operation module: Obtém os saldos da conta identificada por accountId."""
...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/accounts/{account_id}/balances".format(
...
) -> ResponseAccountBalances | ResponseError:
    if response.status_code == 200:
        response_200 = ResponseAccountBalances.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseAccountBalances | ResponseError]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém os saldos da conta identificada por accountId.
     ...
     Método para obter os saldos da conta de depósito à vista, poupança ou pagamento pré-paga
    identificada por accountId mantida pelo cliente na instituição transmissora.
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: UUID,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseAccountBalances | ResponseError | None:
    """Obtém os saldos da conta identificada por accountId.
