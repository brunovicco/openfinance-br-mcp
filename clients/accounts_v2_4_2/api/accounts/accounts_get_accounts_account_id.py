"""Accounts API - accounts operation module: Obtém os dados de identificação da conta identificada por accountId."""
...
) -> ResponseAccountIdentification | ResponseError:
    if response.status_code == 200:
        response_200 = ResponseAccountIdentification.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseAccountIdentification | ResponseError]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém os dados de identificação da conta identificada por accountId.
     ...
     Método para obter os dados de identificação da conta de depósito à vista, poupança ou pagamento pré-
    paga identificada por accountId mantida pelo cliente na instituição transmissora.
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: UUID,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseAccountIdentification | ResponseError | None:
    """Obtém os dados de identificação da conta identificada por accountId.
