...
T = TypeVar("T", bound="CreditCardAccountsTransaction")


@_attrs_define
class CreditCardAccountsTransaction:
    ...
    Attributes:
        transaction_id (str): Código ou identificador único prestado pela instituição que mantém a conta para
            ...
            No entanto, para casos em que a transação ainda está em processamento, é esperado que o `transactionId`
            intermediário seja estável, mudando apenas quando a transação sofrer uma mudança em seu estado.
            Para transações processadas, é esperado que o `transactionld` e demais dados da transação sejam imutáveis.
            O `transactionId` deve obedecer, no mínimo, as regras de imutabilidade propostas conforme a tabela “Data de
            imutabilidade por tipo de transação” presente nas orientações desta API
        ...
        bill_post_date (datetime.date): Data em que a transação foi inserida na fatura. Preencher o campo com a data
            dummy: 0001-01-01, apenas para os casos nos quais ainda não houver a data de inserção na fatura.
             Example: 2021-05-21.
        bill_id (str | Unset): Informação que identifica a fatura onde consta a transação informada. Preencher apenas
            para casos de transação em fatura fechada, ou seja, este campo não é esperado em casos de transação em fatura
            aberta. Example: MTU0OTU1NjI2NTk4OTRmc2ZhZDRmc2Q1NmZkM.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        credit_card_accounts_transaction = cls(
        ...
        credit_card_accounts_transaction.additional_properties = d
        return credit_card_accounts_transaction
