"""Credit Cards Accounts API - credit_card operation module: Conjunto de informações das Contas de pagamento pós paga"""
...
) -> ResponseCreditCardAccountsList | ResponseError:
    if response.status_code == 200:
        response_200 = ResponseCreditCardAccountsList.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseCreditCardAccountsList | ResponseError]:
    return Response(
...
def sync_detailed(
    *,
    ...
    """Conjunto de informações das Contas de pagamento pós paga
     ...
     Método para obter a lista de contas de pagamento pós-paga mantidas pelo cliente na instituição
...
def sync(
    *,
    ...
    x_fapi_interaction_id: UUID,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseCreditCardAccountsList | ResponseError | None:
    """Conjunto de informações das Contas de pagamento pós paga
