"""Credit Cards Accounts API - credit_card operation module: Obtém os limites da conta identificada por creditCardAccountId."""
...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/accounts/{credit_card_account_id}/limits".format(
...
) -> ResponseCreditCardAccountsLimits | ResponseError:
    if response.status_code == 200:
        response_200 = ResponseCreditCardAccountsLimits.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseCreditCardAccountsLimits | ResponseError]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém os limites da conta identificada por creditCardAccountId.
     ...
     Método para obter os limites da conta de pagamento pós-paga identificada por creditCardAccountId
    mantida pelo cliente na instituição transmissora.
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: UUID,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseCreditCardAccountsLimits | ResponseError | None:
    """Obtém os limites da conta identificada por creditCardAccountId.
