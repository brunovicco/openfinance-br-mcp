"""Credit Cards Accounts API - credit_card operation module: Obtém a lista de transações recentes (últimos 7 dias) da conta identificada por creditCardAccountId."""
...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/accounts/{credit_card_account_id}/transactions-current".format(
...
) -> ResponseCreditCardAccountsTransactions | ResponseErrorMetaSingle:
    if response.status_code == 200:
        response_200 = ResponseCreditCardAccountsTransactions.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ResponseCreditCardAccountsTransactions | ResponseErrorMetaSingle]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Obtém a lista de transações recentes (últimos 7 dias) da conta identificada por creditCardAccountId.
     ...
     Método para obter a lista de transações recentes (últimos 7 dias) da conta de pagamento pós-paga
    identificada por creditCardAccountId mantida pelo cliente na instituição transmissora.
...
def sync(
    ...
    *,
    ...
    x_fapi_interaction_id: UUID,
    x_customer_user_agent: str | Unset = UNSET,
) -> ResponseCreditCardAccountsTransactions | ResponseErrorMetaSingle | None:
    """Obtém a lista de transações recentes (últimos 7 dias) da conta identificada por creditCardAccountId.
