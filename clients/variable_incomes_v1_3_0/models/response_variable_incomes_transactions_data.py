"""ResponseVariableIncomesTransactionsData: a data model of the Variable Incomes API (Open Finance Brasil), per the OpenAPI spec's schema for it."""
...
from clients.variable_incomes_v1_3_0.models.enum_variable_incomes_transactions_type import (
...
T = TypeVar("T", bound="ResponseVariableIncomesTransactionsData")


@_attrs_define
class ResponseVariableIncomesTransactionsData:
    """
    Attributes:
        type_ (EnumVariableIncomesTransactionsType): Tipo de movimentação na visão de investimento: entrada ou saída.
            Nos casos de pagamento de dividendos, JCP e aluguéis, fica convencionado que será considerado que o tipo de
            movimento será saída.
             Example: SAIDA.
        transaction_type (EnumVariableIncomesTransactionsTransactionType): O campo deve classificar a movimentação em um
            dos tipos descritos: compra, venda, dividendos, JCP, aluguéis, transferência de custódia, transferência de
            titularidade e outros.
        ...
        transaction_date (datetime.date): Data da movimentação de acordo com a data visualizada pelo cliente nos canais
            da instituição. Data do pregão: compartilhar movimentos até a data da posição.
             Example: 2018-02-15.
        transaction_value (ResponseVariableIncomesTransactionsDataTransactionValue): Valor da operação realizada pelo
            cliente.
        ...
        price_factor (str | Unset): Fator que indica o número de ações utilizadas para a formação do preço. Valor
            informado deve ser maior que zero.
             Example: 100.0005.
        transaction_unit_price (ResponseVariableIncomesTransactionsDataTransactionUnitPrice | Unset): Preço unitário da
            movimentação: valor da unidade do produto na movimentação do investimento. Em transferências de custódia, quando
            o PU original (da instituição de origem) não estiver disponível para a transmissora, deve ser informado o PU
            marcado pela instituição na data de transferência.
    ...
    type_: EnumVariableIncomesTransactionsType
    ...
    transaction_unit_price: (
        'ResponseVariableIncomesTransactionsDataTransactionUnitPrice | Unset'
    ) = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        d = dict(src_dict)
        type_ = EnumVariableIncomesTransactionsType(d.pop("type"))
        ...
        transaction_value = (
            ResponseVariableIncomesTransactionsDataTransactionValue.from_dict(
        ...
        if isinstance(_transaction_unit_price, Unset):
            ...
            transaction_unit_price = (
                ResponseVariableIncomesTransactionsDataTransactionUnitPrice.from_dict(
        ...
        response_variable_incomes_transactions_data = cls(
            type_=type_,
        ...
        return response_variable_incomes_transactions_data
